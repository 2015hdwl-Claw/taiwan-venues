...
// 展開資料（資料是嵌套數組）
const venues = [];
if (Array.isArray(taipeiData)) {
  for (const batch of taipeiData) {
...
} else if (typeof taipeiData === 'object' && taipeiData.venues) {
  venues.push(...taipeiData.venues);
}

// 批次 7 任務指定的場地
const batch7Task = [
  { taskId: 1076, name: '台北寒舍艾美酒店' },
  { taskId: 1077, name: '台北寒舍艾麗酒店' },
  { taskId: 1078, name: '台北寒舍艾麗酒店' }, // 重複
  { taskId: 1079, name: '台北小巨蛋' },
  { taskId: 1081, name: '台北德立莊酒店' }
];
...
function verifyVenue(venue, taskId) {
  ...
  if (venue.venueMainImageUrl && venue.venueMainImageUrl.length > 0) {
    // 檢查是否為第三方來源（Wikipedia、第三方圖庫）
    const isThirdParty = venue.venueMainImageUrl.includes('wikipedia') ||
                          venue.venueMainImageUrl.includes('unsplash') ||
                          venue.venueMainImageUrl.includes('pexels') ||
                          venue.venueMainImageUrl.includes('pixabay');
    if (isThirdParty) {
      result.phase4.error = '照片來源為第三方（Wikipedia/Unsplash），應使用官方來源';
      result.notes.push('Phase 4: 照片來源為第三方');
    } else if (venue.venueMainImageUrl.includes('brandbar') ||
               venue.venueMainImageUrl.includes('logo')) {
      // 檢查是否為 logo 或品牌圖（非實際照片）
      result.phase4.error = '照片為 logo 或品牌圖，非實際場地照片';
      result.notes.push('Phase 4: 照片為 logo 或品牌圖');
    } else {
  ...
  // Phase 5: 品牌/機構場地檢查
  const brandKeywords = ['寒舍', '艾美', '艾麗', '小巨蛋', '德立莊'];
  ...
  if (venue.venueMainImageUrl && venue.images && venue.images.main) {
    ...
    // 只有其中一個存在
    result.phase6.error = 'venueMainImageUrl 和 images.main 其中一個缺失';
    result.notes.push('Phase 6: venueMainImageUrl 和 images.main 其中一個缺失');
  }
...
batch7Task.forEach(task => {
  const venue = findVenueById(task.taskId);
  if (!venue) {
    verificationResults.push({
      ...
      roomsCount: 0,
      status: '待修',
      notes: [`資料庫中未找到 ID ${task.taskId} 的場地`],
...
const outputPath = `./batch7-verify-results-${timestamp}.json`;
