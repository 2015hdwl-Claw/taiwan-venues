...
// 配置：台北喜來登大飯店
const VENUE_NAME = '台北喜來登大飯店';
const VENUE_URL = 'https://www.sheratongrandtaipei.com/websev?lang=zh-tw&ref=pages&cat=50';
...
async function extractVenuePhotos() {
  ...
  try {
    ...
    const meetingKeywords = ['會議', '宴會', 'event', 'meeting', 'ballroom', 'conference', 'banquet', 'grand', '廳', '宴'];
    ...
    if (meetingPhotos.length > 0) {
      ...
      result.gallery = meetingPhotos.slice(1, 11).map(img => img.url);  // 最多 10 張
    ...
    // 12. 保存結果
    const outputPath = '/root/.openclaw/workspace/sheraton_photos.json';
