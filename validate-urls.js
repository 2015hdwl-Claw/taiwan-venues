...
async function main() {
  ...
  try {
    // 讀取場地資料
    const venues = JSON.parse(fs.readFileSync('sample-data-updated.json', 'utf8'));
    ...
    // 驗證所有 URL
    await validator.validateAllUrls(uniqueVenues);
    ...
    console.log('\n✅ 報告已保存到: url-validation-report.json');
    ...
    console.log('✅ 失敗 URL 清單已保存到: failed-urls.json');
