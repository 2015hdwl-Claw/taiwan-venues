...
async function main() {
  ...
  for (const venue of toProcess) {
    ...
    if (results.length % 10 === 0) {
      fs.writeFileSync('batch-crawl-progress-2.json', JSON.stringify(results, null, 2));
    }
  }

  // 保存最終結果
  fs.writeFileSync('batch-crawl-results-2.json', JSON.stringify(results, null, 2));
  console.log('✅ 已保存到 batch-crawl-results-2.json\n');
