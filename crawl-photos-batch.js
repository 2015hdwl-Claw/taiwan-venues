...
async function main() {
  console.log('🚀 批次爬蟲補充照片！\n');

  // 讀取缺少照片的場地
  const venuesNeedingPhotos = JSON.parse(fs.readFileSync('venues-needing-photos.json', 'utf8'));

  console.log(`📊 需要補充照片的場地: ${venuesNeedingPhotos.length} 個\n`);

  // 只處理前 20 個
  const venuesToProcess = venuesNeedingPhotos.slice(0, 20);
  ...
  for (const venue of venuesToProcess) {
    ...
    // 保存中間結果
    fs.writeFileSync('photo-crawl-results.json', JSON.stringify(results, null, 2));

    // 每完成 5 個發送一次進度報告
    if (results.length % 5 === 0) {
      const message = `📊 照片爬蟲進度\n\n` +
        `✅ 已完成: ${results.length}/${venuesToProcess.length}\n` +
  ...
  // 保存總結果
  fs.writeFileSync('photo-crawl-results-final.json', JSON.stringify(results, null, 2));
  console.log('✅ 總結果已保存: photo-crawl-results-final.json\n');
