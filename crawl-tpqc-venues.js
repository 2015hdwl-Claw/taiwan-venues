...
async function crawlVenuePage(url, venueName) {
  console.log(`\n🔍 開始爬蟲: ${venueName}`);
  console.log(`   URL: ${url}\n`);
  ...
  try {
    ...
    const venueInfo = await page.evaluate(() => {
      ...
      const pricePatterns = [
        ...
        /每小時[：:]\s*(\d+(?:,\d+)?)\s*(?:元|NTD|TWD|\$)/,
        /半日[：:]\s*(\d+(?:,\d+)?)\s*(?:元|NTD|TWD|\$)/
      ];
      ...
      // 嘗試提取所有會議室
      const roomElements = document.querySelectorAll('.room-item, .venue-item, .space-item, .conference-room');
...
async function main() {
  console.log('🚀 Playwright 深度爬蟲台灣文創中心場館！\n');

  const venues = [
    {
      name: '台北松江1館',
      url: 'https://www.tpqc.com.tw/場地租用/台北場館/台北松江1館/'
    },
    {
      name: '新北板橋館',
      url: 'https://www.tpqc.com.tw/場地租用/tccc新北場館/新北板橋館/'
    },
    {
      name: '台北松江2館',
      url: 'https://www.tpqc.com.tw/場地租用/台北場館/台北松江2館/'
    },
    {
      name: '桃園站前館',
      url: 'https://www.tpqc.com.tw/場地租用/桃園場館/桃園站前館/'
    }
  ];
  ...
  for (const venue of venues) {
    const result = await crawlVenuePage(venue.url, venue.name);
    results.push(result);
  ...
  // 保存總結果
  fs.writeFileSync('tpqc-venues-playwright.json', JSON.stringify(results, null, 2));
  console.log('✅ 總結果已保存: tpqc-venues-playwright.json\n');
