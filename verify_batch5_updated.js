...
// 批次 5 場地列表（更新正確的官網 URL）
const venues = [
  {
    id: 83960,
    name: "台北香格里拉遠東國際大飯店",
    // 正確的台北分店官網
    url: "https://www.shangri-la.com/taipei/shangrila/",
    city: "台北市"
  },
  {
    id: 63277,
  ...
  },
  {
    ...
    name: "台北喜來登大飯店",
    // 正確的台北分店官網
    url: "https://www.sheraton-taipei.com/",
    city: "台北市"
  },
  {
    id: 1706,
    name: "台北喜瑞飯店",
    // 正確的官網 URL
    url: "https://www.ambiencehotel.com.tw/",
    city: "台北市"
  },
  {
    id: 2505,
...
async function verifyVenue(browser, venue) {
  const result = {
    ...
    roomsCount: 0,
    status: '待修',
    notes: venue.notes ? [venue.notes] : [],
    phases: {}
  };

  // 如果官網 URL 為 null，直接標記為下架
  if (!venue.url) {
    console.log(`\n${'='.repeat(60)}`);
    console.log(`驗證場地: ${venue.name} (ID: ${venue.id})`);
    console.log('='.repeat(60));
    console.log(`  ❌ 官網不存在或已歇業`);
    result.status = '下架';
    result.phases.phase1 = 'N/A';
    return result;
  }
  ...
  try {
    ...
    if (meetingLinks.length > 0) {
      ...
      meetingLinks.slice(0, 5).forEach((link, i) => {
        console.log(`    ${i+1}. ${link.text.substring(0, 30)} → ${link.href.substring(0, 60)}`);
      });
    ...
    if (meetingLinks.length > 0) {
      ...
      try {
        ...
        const rooms = await page.evaluate(() => {
          ...
          // 尋找包含會議室資訊的元素
          const roomElements = document.querySelectorAll('[class*="room"], [class*="venue"], [class*="ballroom"], [class*="hall"], [class*="space"]');
          
          return {
            count: Math.max(roomElements.length, roomNames.length, 1),
            names: roomNames
          };
        });
...
async function main() {
  ...
  // 輸出結果
  console.log('\n\n' + '='.repeat(60));
  console.log('批次 5 驗證結果總結（更新版）');
  console.log('='.repeat(60));
  
  results.forEach(r => {
    console.log(`\nID: ${r.id}`);
    console.log(`名稱: ${r.name}`);
    console.log(`URL: ${r.url || '(無官網)'}`);
  ...
  // 保存結果
  const outputPath = '/root/.openclaw/workspace/taiwan-venues/batch5_verification_results_updated.json';
