...
const toVerify = JSON.parse(fs.readFileSync('batch1-verify-list.json', 'utf8'));

console.log('=== SOP v3.0 批次驗證：新北市、台南市、桃園市 ===');
...
async function main() {
  ...
  fs.writeFileSync(`batch1-verify-final-${timestamp}.json`, JSON.stringify(results, null, 2));
  fs.writeFileSync('venues-all-cities.json', JSON.stringify(data, null, 2));
