...
const toVerify = JSON.parse(fs.readFileSync('taipei-verify-list.json', 'utf8'));

console.log('=== SOP v3.0 台北市驗證 ===');
...
async function verifyUrl(browser, info, index, total) {
  ...
  console.log('場地數:', info.venues.length);
  ...
  try {
    ...
    if (!titleMatch) {
      console.log('  ⚠️ 標題不完全匹配');
      console.log('    場地:', info.venues[0].name);
    ...
    if (meetingLinks.length > 0) {
      ...
      try {
        ...
        console.log('  ⚠️ 無法開啟會議室頁面:', e.message.slice(0, 50));
...
async function main() {
  ...
  for (let i = 0; i < toVerify.length; i += batchSize) {
    ...
    fs.writeFileSync(`taipei-verify-progress-${timestamp}.json`, JSON.stringify(results, null, 2));
  ...
  // 儲存最終結果
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  fs.writeFileSync(`taipei-verify-final-${timestamp}.json`, JSON.stringify(results, null, 2));
  fs.writeFileSync('venues-all-cities.json', JSON.stringify(data, null, 2));
