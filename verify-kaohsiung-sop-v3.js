...
const toVerify = JSON.parse(fs.readFileSync('kaohsiung-verify-list.json', 'utf8'));

console.log('=== SOP v3.0 高雄市驗證 ===');
...
async function main() {
  ...
  for (let i = 0; i < toVerify.length; i += batchSize) {
    ...
    fs.writeFileSync(`kaohsiung-verify-progress-${timestamp}.json`, JSON.stringify(results, null, 2));
  ...
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  fs.writeFileSync(`kaohsiung-verify-final-${timestamp}.json`, JSON.stringify(results, null, 2));
  fs.writeFileSync('venues-all-cities.json', JSON.stringify(data, null, 2));
