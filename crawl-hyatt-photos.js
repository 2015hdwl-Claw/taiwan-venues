...
// 配置：君悅酒店
const VENUE_NAME = '台北君悅酒店';
const VENUE_URL = 'https://www.grandhyatttaipei.com/meetings-events/';
...
async function extractVenuePhotos() {
  ...
  try {
    ...
    const meetingKeywords = ['會議', '宴會', 'event', 'meeting', 'ballroom', 'conference', 'banquet', 'grand', '廳'];
    ...
    if (heroImages.length > 0) {
      ...
      // 如果沒有背景圖片，使用第一張大圖
      result.mainHero = loadedImages.find(img => img.width > 500) || loadedImages[0];
    }
    ...
    if (result.mainHero) {
      console.log(`\n🎯 主 Hero 圖片: ${result.mainHero.url}`);
    }
    
    // 12. 保存結果
    const fs = require('fs');
    const outputPath = '/root/.openclaw/workspace/taiwan-venues/hyatt-taipei-photos.json';
